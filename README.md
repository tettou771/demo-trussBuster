...
（NEEDLE EYE → LIMBO → ARROW SLIT → CATAPULT → GAUNTLET）。
**壊せない静的な壁**が登場し、弾道そのものが問われる：影壁に隠れた的は隣の幅広スラブを
撃って巻き込む、天井の下はMAXのフラット弾道でしか通らない、狭間（スリット）を通す、
壁越しに山なりロブ、シーソーの手前に落として荷物を射出 — など。
...
- 描画は GPU PBR ではなく **CPU ライティング**（`Mesh::drawWithLighting`、sokol_gl 経由）。
  deferred PBR が HUD テキストを上書きする未解明の描画順問題の回避 + フラットで明るい
  ルックの採用（`FlatRenderer.h` 参照。なお iOS Safari で壊れるのは IBL ベイクだけで
  PBR 自体は動く — core が自動スキップ済み）
