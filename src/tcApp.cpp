...
void tcApp::drawDebugPanel() {
    ...
    ImGui::Text("phase: %s  level: %d  score: %d",
                phaseName(scene_->getPhase()), scene_->getLevelNumber(), scene_->getScore());
    ImGui::Text("shots: %d  blocks: %d/%d  balls: %d",
                scene_->getShots(), scene_->getBlocksLeft(), scene_->getBlocksTotal(),
                scene_->aliveBalls());
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        if (ImGui::Button(("L" + to_string(i)).c_str())) scene_->gotoLevel(i);
    }
...
void tcApp::registerMcpTools() {
    ...
    mcp::tool("goto_level", "Jump straight to a level (1-5), for debugging")
        .arg<int>("level", "level number 1-5")
        .bind([this](const json& a) {
            scene_->gotoLevel(a.value("level", 1));
            return scene_->stateJson();
        });
